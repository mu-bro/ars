...
class Spreadsheet_Excel_Reader
{
    ...
    function setReadLineType($type)
    {
	$this->_readLineType = $type;
    }
...
//                                              echo "Str $i = $retstr\n";
        ...
        foreach ($this->boundsheets as $key=>$val){
            $this->sn = $key;
            if( !$this->_readLineType ) {
	            $this->_parsesheet($val['offset']);
	    } else {
		    $this->currentOffset = array();
	    }
        }
        return true;
    ...
    function RewindRow( $sheets ){
	if( !empty( $this->currentOffset[ $sheets ] ) ) {
		$this->boundsheets[ $sheets ]["offset"] = $this->currentOffset[ $sheets ];
		unset( $this->sheets[ $sheets ]["cells"] );
	}
    }
    
    function nextRow( $sheets ){
	$spos = $this->boundsheets[ $sheets ]["offset"];
	$cont = true;
        // read BOF
	...
	if( empty( $this->currentOffset[ $sheets ] ) ) {
	        ...
	        $version = $this->version = ord($this->data[$spos + 4]) | ord($this->data[$spos + 5])<<8;
	        ...
	        $this->currentOffset[ $sheets ] = $this->boundsheets[ $sheets ]["offset"];
	...
	while($cont) {
	    ...
	    if( $_row === false && !empty( $this->sheets[ $sheets ]["cells"] ) ) {
		// Не загружаем пямять
		unset( $this->sheets[ $sheets ]["cells"] );
	    }
...
//             unset($this->rectype);
            ...
            switch ($code) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_RK2:
                    ...
                    if( $_row === false || $_row === $row ) {
			    $_row = $row;
			    $this->addcell($row, $column, $string, $raw);
                    } else {
			    $return = array( $_row + $this->_rowoffset => $this->sheets[$sheets]["cells"][ $_row + $this->_rowoffset ] );
                    }
		    ...
		    $spos += $length;
		    $this->boundsheets[ $sheets ]["offset"] = $spos;
	    }
...
// 		$this->dataFileName = DIR_APPLICATION . "temp/" . ( rand(0,999999) * time() . ".tmp" );
// 		file_put_contents( $this->dataFileName, $this->data  );
